...
typedef struct {
    ...
    float delayBuffer[FRAMES_PER_BUFFER * STEREO];
} slice;
        ...
        PaStreamCallbackFlags statusFlags, void *userData ) 
{
    ...
    SAMPLE * left = (SAMPLE *)outputBuffer;

    int i, j, readcountA, readcountB, readcountC, readcountD;

    /* read data from file*/
    readcountA = sf_readf_float (data.sliceA.infile, data.sliceA.buffer, framesPerBuffer); 
    readcountB = sf_readf_float (data.sliceB.infile, data.sliceB.buffer, framesPerBuffer);
    ...
    if (!data.sliceB.playing) {
        ...
        readcountB = sf_readf_float (data.sliceB.infile, data.sliceB.buffer+(readcountB*STEREO), framesPerBuffer-readcountB);
    ...
    for (i = 0; i < framesPerBuffer * STEREO; i+=2) {
        out[i] = ((data.sliceA.volume * data.sliceA.buffer[i] * data.sliceB.muter) + (data.sliceB.volume * data.sliceB.buffer[i] * data.sliceB.muter) + (data.sliceC.volume * data.sliceC.buffer[i] * data.sliceC.muter) + (data.sliceD.volume * data.sliceD.buffer[i] * data.sliceD.muter));
    ...
    /* FILTERSSS */
    /* STFT */
    /*for (i = 0; i < (framesPerBuffer*STEREO); i+=HOP_SIZE)
    {
        ...
        /* Filter windows */
        /*int lowpass = data.lowpass;
        int hipass = WINDOW_SIZE/4 - data.highpass;
        for (j = 0; j < WINDOW_SIZE/2; ++j) {
...
void keyboardFunc( unsigned char key, int x, int y )
{
    ...
    switch( key )
    {
        ...
        case 'r':
            data.lowpass += LOWPASS_INCR;
            if (data.lowpass > WINDOW_SIZE/2) {
                data.lowpass = WINDOW_SIZE/2;
            }
            break;

        case 'u':
            data.highpass -= HIGHPASS_INCR;
            if (data.highpass < 0) {
                data.highpass = 0;
            }
            break;
        case 'i':
            data.highpass += HIGHPASS_INCR;
            if (data.highpass > WINDOW_SIZE/2) {
                data.highpass = WINDOW_SIZE/2;
            } 
            break;
...
void increaseLoopLength(int length)
{
    switch (data.sliceSelector){
        case 0:
            if (data.sliceA.loopLength < data.sliceA.sfinfoInput.frames - INC_LOOP_LENGTH){//Only increase if loop length will be smaller than total audio
                data.sliceA.loopLength += INC_LOOP_LENGTH;
            }
            break;
        
        case 1:
            if (data.sliceB.loopLength < data.sliceB.sfinfoInput.frames - INC_LOOP_LENGTH){//Only increase if loop length will be smaller than total audio
                data.sliceB.loopLength += INC_LOOP_LENGTH;
            }
            break;

        case 2:
            if (data.sliceC.loopLength < data.sliceC.sfinfoInput.frames - INC_LOOP_LENGTH){//Only increase if loop length will be smaller than total audio
                data.sliceC.loopLength += INC_LOOP_LENGTH;
            }
            break;
        
        case 3:
            if (data.sliceD.loopLength < data.sliceD.sfinfoInput.frames - INC_LOOP_LENGTH){//Only increase if loop length will be smaller than total audio
